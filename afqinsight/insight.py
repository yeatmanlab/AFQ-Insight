...
def inner_cv_classify(x_train, y_train, x_test, y_test,
    ...
    if save_results:
        ...
        cv_df.to_pickle(f'grid_results_{save_suffix}.pkl')

    if save_model:
        # serialize model to JSON
        model_json = best_model.to_json()
        with open(f'model_{save_suffix}.json', "w") as json_file:
            json_file.write(model_json)

        # serialize weights to HDF5
        best_model.save_weights(f'model_weights_{save_suffix}.hdf5')
