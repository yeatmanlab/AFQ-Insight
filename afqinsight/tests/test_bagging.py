"""
Testing for the serial bagging ensemble module (afqinsight._serial_bagging).
"""
...
def test_error():
    ...
    # Test max_samples
    assert_raises(ValueError, SerialBaggingClassifier(base, max_samples=-1).fit, X, y)
    assert_raises(ValueError, SerialBaggingClassifier(base, max_samples=0.0).fit, X, y)
    assert_raises(ValueError, SerialBaggingClassifier(base, max_samples=2.0).fit, X, y)
    assert_raises(ValueError, SerialBaggingClassifier(base, max_samples=1000).fit, X, y)
    assert_raises(
        ValueError, SerialBaggingClassifier(base, max_samples="foobar").fit, X, y
    )

    # Test max_features
    assert_raises(ValueError, SerialBaggingClassifier(base, max_features=-1).fit, X, y)
    assert_raises(ValueError, SerialBaggingClassifier(base, max_features=0.0).fit, X, y)
    assert_raises(ValueError, SerialBaggingClassifier(base, max_features=2.0).fit, X, y)
    assert_raises(ValueError, SerialBaggingClassifier(base, max_features=5).fit, X, y)
    assert_raises(
        ValueError, SerialBaggingClassifier(base, max_features="foobar").fit, X, y
    )
...
def test_base_estimator():
    ...
    ensemble = SerialBaggingClassifier(None, n_jobs=3, random_state=0).fit(
        X_train, y_train
    )
    ...
    ensemble = SerialBaggingRegressor(None, n_jobs=3, random_state=0).fit(
        X_train, y_train
    )
...
def test_bagging_with_pipeline():
    estimator = SerialBaggingClassifier(
...
def test_bagging_sample_weight_unsupported_but_passed():
    estimator = SerialBaggingClassifier(DummyZeroEstimator())
    rng = check_random_state(0)
...
def test_warm_start(random_state=42):
    ...
    for n_estimators in [5, 10]:
        if clf_ws is None:
            clf_ws = SerialBaggingClassifier(
    ...
    clf_no_ws = SerialBaggingClassifier(
...
def test_warm_start_smaller_n_estimators():
    ...
    X, y = make_hastie_10_2(n_samples=20, random_state=1)
    clf = SerialBaggingClassifier(n_estimators=5, warm_start=True)
    clf.fit(X, y)
...
def test_warm_start_equal_n_estimators():
    ...
    clf = SerialBaggingClassifier(n_estimators=5, warm_start=True, random_state=83)
...
def test_warm_start_equivalence():
    ...
    clf_ws = SerialBaggingClassifier(n_estimators=5, warm_start=True, random_state=3141)
    clf_ws.fit(X_train, y_train)
    clf_ws.set_params(n_estimators=10)
    clf_ws.fit(X_train, y_train)
    y1 = clf_ws.predict(X_test)

    clf = SerialBaggingClassifier(n_estimators=10, warm_start=False, random_state=3141)
    clf.fit(X_train, y_train)
...
def test_warm_start_with_oob_score_fails():
    ...
    X, y = make_hastie_10_2(n_samples=20, random_state=1)
    clf = SerialBaggingClassifier(n_estimators=5, warm_start=True, oob_score=True)
    assert_raises(ValueError, clf.fit, X, y)
...
def test_oob_score_removed_on_warm_start():
    ...
    clf = SerialBaggingClassifier(n_estimators=50, oob_score=True)
    clf.fit(X, y)
...
def test_oob_score_consistency():
    ...
    bagging = SerialBaggingClassifier(
        KNeighborsClassifier(),
        max_samples=0.5,
        max_features=0.5,
        oob_score=True,
...
def test_set_oob_score_label_encoding():
    ...
    Y3 = [0, 1, 2] * 5
    x1 = (
        SerialBaggingClassifier(oob_score=True, random_state=random_state)
        .fit(X, Y1)
        .oob_score_
    )
    x2 = (
